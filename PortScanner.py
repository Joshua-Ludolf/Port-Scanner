...
import socket
import tkinter as tk        # this will import the tkinter library and the "as tk" part makes it easier for the program to call from tkinter library
            ...
            if Timeout <= 1:
                ...
                for APN in PNDnary.keys():
                    ...
                    if (ComputerCommunitcation.connect_ex((TargetIP, APN))) == 0:
                        MsgBoxStr += (f"IP Address: {IP} , Port Number: {APN} , Port Name: {PNDnary[APN][0]} , Port Description: {PNDnary[APN][1]}\n")                
                    else:
                        MsgBoxStr += ""
                ...
                MsgBoxStr = (f"Timeout {Timeout} is too long as it is greater than 1 second and is a really long time in the real world!")
                stMessageBox.insert(tk.INSERT, MsgBoxStr)
                Mainwin.update_idletasks()
    else:
...
def ScanIP():
    ...
    if IP.find('.') == 3:
       ...
       MsgBoxStr = "Invalid IP Address, IP must have three decimals (no more and no less) and must contain numbers (float or integers)!"
       stMessageBox.insert(tk.INSERT, MsgBoxStr)
       Mainwin.update_idletasks()
...
def CreateMainWindow():
    ...
    btnClear.grid(row=4, column=2, sticky="NW") 

    stMessageBox = tk.scrolledtext.ScrolledText(Mainwin,width=30,height=8)
    stMessageBox.grid(row=2,column=1, columnspan=2,rowspan=2)
    
    btnRead = tk.Button(Mainwin, text="Read Ports", command=ReadList)
    btnRead.grid(row=4, column=1, sticky="NW")

    btnScan = tk.Button(Mainwin, text="Scan IP Num",command=ScanIP)
    btnScan.grid(row=4, column=2, sticky="NW")
...
def main():
    ...
    CreateMainWindow()
    Mainwin.mainloop()  # This is the call that keeps painting the main window on the screen forever - until user kills the program
